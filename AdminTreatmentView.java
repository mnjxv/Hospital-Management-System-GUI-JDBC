...
public class AdminPatientView extends JFrame {
    ...
    private static final String[] COLUMNS = {
        "Patient ID","Name","Gender","Address","Contact Number","Date of Birth","Medical History","Patient Type", "Room ID"
    };
    ...
    private static final java.util.List<AdminPatientView> openViews = new java.util.ArrayList<>();

    public static void refreshAllPatientLists() {
        for (AdminPatientView view : openViews) {
            try {
    ...
    private void buildTable() {
        ...
        table.setRowHeight(24); table.setFont(UI);
    ...
    public void clearFields() {
        txtName.setText(""); dobPicker.clear(); cmbGender.setSelectedIndex(-1);
        txtContact.setText(""); txtAddress.setText(""); txtMedHistory.setText("");
        cmbPatientType.setSelectedIndex(-1); cmbRoom.setSelectedIndex(-1); table.clearSelection();
    }
    ...
    // ── CalendarButton — styled as a fake JComboBox with proper ▼ arrow ──────
    public static class CalendarButton extends JPanel {
        ...
        public CalendarButton() {
            ...
            // Replace the UI so the arrow button opens the calendar
            ...
            // Renderer shows the picked date
            fakeCombo.setRenderer(new DefaultListCellRenderer() {
                @Override
                public Component getListCellRendererComponent(
                        JList<?> list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
        ...
        /** Returns ISO date string (yyyy-MM-dd), or "" if none selected. */
        public String getDate() {
            return date == null ? "" : date.format(SQL);
        }

        /** Accepts ISO date string (yyyy-MM-dd) or datetime with space/T separator. */
